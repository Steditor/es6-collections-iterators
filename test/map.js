...
describe('Map prototype functions', function(){
  ...
  it('should iterate over entries of a filled map', function() {
    ...
    expect(first.done).to.be.false;
    expect(first.value).to.deep.equal(['a', 1]);

    expect(second.done).to.be.false;
    expect(second.value).to.deep.equal(['b', 2]);

    expect(third.done).to.be.false;
    expect(third.value).to.deep.equal(['c', 3]);
  ...
  it('should iterate over entries of an empty map', function() {
    var iterator = emptymap.entries();
    var first = iterator.next();
    var second = iterator.next();
